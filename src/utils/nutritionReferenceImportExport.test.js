...
describe('nutritionReferenceImportExport', () => {
  test('exports rows as semicolon CSV with synonym list', () => {
    ...
    expect(csv).toContain('ingredientID;family;category;synonyms;defaultAmountG;kalorien;protein;fett;kohlenhydrate;zucker;ballaststoffe;salz');
    expect(csv).toContain('dummy-tomate;Gemüse;Nachtschatten;Tomate|Paradeiser;100;18');
  });

  test('parses imported CSV rows and validates required fields', () => {
    const rows = parseNutritionReferenceCsv(
      [
        'ingredientID;family;category;synonyms;defaultAmountG;kalorien',
        'dummy-kartoffel;Gemüse;Knolle;Kartoffel|Erdapfel;150;86',
      ].join('\n')
    );
